...
class ConsulRoleModule(_ConsulModule):
    ...
    def map_param(self, k, v, is_update):
        # remove aliases, TODO: Can we do this globally in _ConsulModule instead?
        if k in {"node_identities", "service_identities"} and v:
            v = [{key: value for key, value in i.items() if key != "name"} for i in v]
