...
options:
  ...
  max_token_ttl:
    description:
      ...
      - Can be specified in the form of "60s" or "5m" (i.e., 60 seconds or 5 minutes, respectively).
    type: str
  token_locality:
    description:
...
def normalize_ttl(ttl):
    ...
    if hours:
        new_ttl += "{}h".format(hours)
    minutes, seconds = divmod(remainder, 60)
    if minutes:
        new_ttl += "{}m".format(minutes)
    if seconds:
        new_ttl += "{}s".format(seconds)
    return new_ttl
